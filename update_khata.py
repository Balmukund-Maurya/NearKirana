  ...
  Future<void> _fetchCustomers() async {
    ...
    try {
      ...
      if (_searchQuery.isNotEmpty) {
        // Simple search by name prefix or exact phone (Requires exact match or prefix)
        // Since we didn't add searchKeywords to customers yet, we will just use basic ordering and filtering on client side if needed,
        // or just rely on a "name" array if we had one.
        // Wait, for customers, let's just fetch them and filter locally if there's a search query, 
        // OR better yet, implement a `name` >= `_searchQuery` & `name` <= `_searchQuery` + '\uf8ff'.
...
# We need to replace everything from "class KhataScreen" to "Widget build(BuildContext context) {"
new_content = re.sub(r'class KhataScreen extends StatelessWidget \{.*Widget build\(BuildContext context\) \{', header, content, flags=re.DOTALL)
        ...
        children: [
          Padding(
            ...
            child: TextField(
              ...
              onChanged: (val) {
                // Capitalize first letter to match common names if needed
                setState(() {
                                      ...
                                      children: [
                                        ...
                                        Switch(
                                          ...
                                          onChanged: (bool val) async {
                                            ...
                                            // Refresh local item
                                            setState(() {
                                              // we could manually update it here if we want, or rely on pull to refresh
                                              // since we removed StreamBuilder, let's just do a pull to refresh
                                            });
                                            _refresh();
                                          },
                                        ),
...
new_content = re.sub(r'    return Scaffold\(.*      body: StreamBuilder<QuerySnapshot>\(.*      \),\n    \);\n  \}', body_replacement, new_content, flags=re.DOTALL)
